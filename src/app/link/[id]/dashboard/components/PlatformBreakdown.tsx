  ...
  if (platforms.length === 0) {
    return (
      <div className="flex flex-col gap-[16px] rounded-[8px] border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark p-[24px] items-center justify-center min-h-[200px]">
        <p className="text-[14px] text-subtext-light">OS 데이터가 없습니다.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-[24px] rounded-[8px] border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark p-[24px]">
      ...
      <div className="flex flex-col gap-[20px]">
        {sortedStats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="flex flex-col gap-[8px] group">
              ...
              {/* 프로그레스 바 배경 */}
              <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-[9999px] h-[8px] overflow-hidden">
                <div
                  className={`h-full rounded-[9999px] transition-all duration-1000 ease-out ${config.color}`}
                  style={{
      ...
      {/* 전체 요약 (선택 사항) */}
      <div className="pt-[8px] border-t border-border-light dark:border-border-dark flex justify-between items-center">
